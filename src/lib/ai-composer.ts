...
【起名忌用字 - 绝对禁止使用以下任何字】
${intent.gender === "F" ? "女性忌用：婵（孤单）、伶（艺伎婚姻不顺）、娥（孤寡克夫）、花（桃花煞）、芬（感情波折）、洁（婚姻不顺）、娟（婚姻不满）、洪（大起大落）、贞（过于刚烈）、薄（薄情）、冶（妖冶）、媚（谄媚）、娇（娇弱过度依赖）" : ""}
${intent.gender === "M" ? "男性忌用：艳（过于女性化）、娇（过于柔弱）、媚（谄媚）、妖（不正）、薄（薄情寡义）、邪（邪恶）、戾（暴戾）、狡（狡猾）、伪（虚伪）、奸（奸诈）" : ""}
男女皆忌：死、亡、灭、绝、鬼、魔、妖、邪、杀、弑、刽、戮、疯、癫、痴、傻、病、疾、瘟、痨、残、废、缺、哀、悲、苦、惨、孤、寡、独
...
⚠️【最高优先级 - 必须严格遵守】
你生成的名字中的【每一个字】都必须来自上面的候选字池！
禁止使用任何不在候选字池中的字！如果必须使用字池外的字，请用字池中的同义字替换。
例如：不能用"简"，因为它不在字池中；可以用字池中存在的字如"沐""涵""汐"等。

请生成 ${config.maxCandidates} 个符合要求的名字。
【硬性约束 - 必须全部满足】
