 ...
 * 显示支付二维码并轮询支付结果
 */
export default function WxpayQRCode({
  orderNo,
  codeUrl,
  amount,
  onSuccess,
  ...
  useEffect(() => {
    ...
    // 使用 goeasy 或其他 QRCode API
    // 这里我们使用前端生成，或者直接使用微信支付原生的 code_url
  ...
  const checkPaymentStatus = useCallback(async () => {
    try {
      ...
      const res = await fetch(`/api/wxpay/query?orderNo=${orderNo}`, {
        headers: {
  ...
  const refreshQRCode = async () => {
    ...
    try {
      const res = await fetch("/api/wxpay/create-order", {
        ...
        body: JSON.stringify({
          tier: parseInt(orderNo.match(/^WX(\d)/)?.[1] || "1"),
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 28, fontWeight: 700, color: "#07c160" }}>
          ¥{amount.toFixed(2)}
        </div>
      </div>
      ...
      >
        <div style={{ marginBottom: 4 }}>请使用微信扫一扫</div>
      ...
      !qrCodeUrl && (
        <button
          onClick={async () => {
            const token = localStorage.getItem("token");
            await fetch("/api/wxpay/mock-complete", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
