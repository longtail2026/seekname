#!/usr/bin/env python3
"""
专门为naming_classics表生成BGE-M3嵌入向量 - 只处理新增条目
针对新增的2099条记录进行向量化，不清除现有嵌入向量
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('naming_classics_vectorization_new.log'),
...
def main():
    ...
    try:
        ...
        # 首先检查当前状态
        logger.info("检查naming_classics表当前状态...")
        vectorizer.cur.execute("SELECT COUNT(*) FROM naming_classics")
        total_entries = vectorizer.cur.fetchone()[0]
        logger.info(f"总条目数: {total_entries}")
        
        vectorizer.cur.execute("SELECT COUNT(combined_text_embedding) FROM naming_classics WHERE combined_text_embedding IS NOT NULL")
        vectorized_entries = vectorizer.cur.fetchone()[0]
        logger.info(f"已向量化条目数: {vectorized_entries}")
        logger.info(f"待向量化条目数: {total_entries - vectorized_entries}")
        
        # 处理所有条目（只处理未向量化的）
        logger.info("开始为naming_classics表新增条目生成嵌入向量...")
        logger.info(f"注意: 只处理未向量化的 {total_entries - vectorized_entries} 条记录，不清除现有嵌入向量")
        logger.info("预计时间: 根据硬件性能，可能需要数小时")
        
        vectorizer.process_all(
            batch_size=16,  # 较小的批处理大小以适应CPU内存
            clear_existing=False  # 不清除现有嵌入向量，只处理新增条目
        )
        ...
        logger.info("\nnaming_classics表新增条目向量化完成!")
        logger.info("\n总结:")
        logger.info(f"1. 处理新增的 {total_entries - vectorized_entries} 条记录")
        logger.info("2. 使用BGE-M3模型在CPU上运行")
        logger.info("3. 生成古籍、现代和组合嵌入向量")
        logger.info("4. 存储为PostgreSQL bytea格式")
        logger.info("5. 保留现有嵌入向量，只处理新增条目")
