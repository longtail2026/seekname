...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('bge_m3_embeddings_optimized.log'),
...
class BGE_M3_Vectorizer_Optimized:
    """使用BGE-M3向量化典籍条目 - 优化版本"""
    ...
    def generate_embeddings_batch(self, entries: List[Tuple[int, str, Optional[str]]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        # 生成嵌入向量 - 使用更大的批大小
        ancient_embeddings = self.embedder.encode(ancient_texts, batch_size=16)
        modern_embeddings = self.embedder.encode(modern_texts, batch_size=16) if any(modern_texts) else np.array([])
    ...
    def generate_embeddings_multiprocess(self, entries: List[Tuple[int, str, Optional[str]]]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """多进程生成嵌入向量"""
        if self.num_workers <= 1:
            return self.generate_embeddings_batch(entries)
        
        # 分割数据到各个工作进程
        chunk_size = max(1, len(entries) // self.num_workers)
        chunks = []
        for i in range(0, len(entries), chunk_size):
            chunks.append(entries[i:i + chunk_size])
        
        logger.info(f"使用 {len(chunks)} 个工作进程处理 {len(entries)} 个条目")
        ...
        with ProcessPoolExecutor(max_workers=min(self.num_workers, len(chunks))) as executor:
            ...
            for future in as_completed(future_to_chunk):
                ...
                try:
                    ...
                    logger.error(f"工作进程 {chunk_idx} 失败: {e}")
                    raise
        ...
        for _, (ancient_emb, modern_emb, combined_emb) in results:
            all_ancient.append(ancient_emb)
            if modern_emb is not None and len(modern_emb) > 0:
                all_modern.append(modern_emb)
            all_combined.append(combined_emb)
        
        ancient_embeddings = np.vstack(all_ancient) if all_ancient else np.array([])
        modern_embeddings = np.vstack(all_modern) if all_modern else np.array([])
        combined_embeddings = np.vstack(all_combined) if all_combined else np.array([])
        
        return ancient_embeddings, modern_embeddings, combined_embeddings
    
    def _process_chunk(self, chunk: List[Tuple[int, str, Optional[str]]], chunk_idx: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """处理单个数据块（在工作进程中运行）"""
        # 在工作进程中创建新的模型实例
        ...
        combined_embeddings = []
        for i, (ancient_vec, modern_vec) in enumerate(zip(ancient_embeddings, modern_embeddings if len(modern_embeddings) > 0 else [None] * len(ancient_embeddings))):
            if modern_vec is not None and np.any(modern_vec):
                combined = 0.7 * ancient_vec + 0.3 * modern_vec
                norm = np.linalg.norm(combined)
                if norm > 0:
                    combined = combined / norm
    ...
    def process_all(self, batch_size: int = 32, clear_existing: bool = False, use_multiprocess: bool = False):
        """处理所有条目 - 优化版本"""
        if clear_existing:
        ...
        try:
            while True:
                ...
                # 生成嵌入向量（可选择使用多进程）
                if use_multiprocess and self.num_workers > 1 and len(entries) >= 20:
                    ancient_embeddings, modern_embeddings, combined_embeddings = \
                        self.generate_embeddings_multiprocess(entries)
                else:
                    ancient_embeddings, modern_embeddings, combined_embeddings = \
...
def main():
    ...
    try:
        ...
        # 初始化向量化器
        vectorizer = BGE_M3_Vectorizer_Optimized(
            host=host,
            port=port,
            database=database,
            user=user,
        ...
        # 参数说明：
        # batch_size=32: 每次从数据库获取32条记录
        # clear_existing=True: 清除现有嵌入向量
        # use_multiprocess=True: 使用多进程加速嵌入生成
        ...
        logger.info("3. 添加多进程支持，使用{}个工作进程".format(num_workers))
        logger.info("4. 优化内存管理和缓存清理")
        logger.info("\n预期性能提升:")
        logger.info("- 单批处理时间减少30-50%")
        logger.info("- 总体处理时间预计减少40-60%")
        logger.info("- 内存使用更高效")
